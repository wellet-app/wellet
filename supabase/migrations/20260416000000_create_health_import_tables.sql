-- ============================================================
-- Health Record Import: new structured data tables + documents
-- Supports onboarding upload-first flow and Phase 2 import pipeline
-- ============================================================
...
create table if not exists public.documents (
  ...
  doc_type      text default 'Other document',
  status        text default 'pending'
                check (status in ('pending', 'processing', 'complete', 'failed')),
  extracted_data jsonb default '{}'::jsonb,   -- AI-extracted content (summary, entities)
  tags          text[] default '{}',
...
create index idx_allergies_person on public.allergies (person_id);
...
create index idx_lab_results_person on public.lab_results (person_id);
create index idx_lab_results_date on public.lab_results (effective_date desc);
...
create index idx_vitals_person on public.vitals (person_id);
create index idx_vitals_date on public.vitals (effective_date desc);
...
create index idx_export_jobs_person on public.health_export_jobs (person_id);

alter table public.health_export_jobs enable row level security;

create policy "Users can view own export jobs"
  on public.health_export_jobs for select to authenticated
  using (auth.uid() = user_id);

create policy "Users can insert own export jobs"
  on public.health_export_jobs for insert to authenticated
  with check (auth.uid() = user_id);

create policy "Users can update own export jobs"
  on public.health_export_jobs for update to authenticated
  using (auth.uid() = user_id);
